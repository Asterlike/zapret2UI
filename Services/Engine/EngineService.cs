...
/// </summary>
public sealed partial class EngineService : IDisposable
{
        ...
        bool coverTgProxy = false, bool debugLog = false, bool forLaunch = false)
    {
        ...
        // Cloudflare's WARP service domains, always. Not a setting: it is what makes the WARP tab's first
        // step possible at all, and a user cannot be expected to find a toggle for a dependency they have
        // no way of knowing about. Scoped to one hostlist, so it is inert for everyone else.
        // WARP's own MASQUE entry points, always, and BEFORE the profile above. Profiles are scanned in
        // order and the first whose filter matches wins, so this has to precede the by-name one — which
        // would otherwise swallow the same connections and hand them the gentler pipeline.
        AppendMasqueCoverage(result);
        AppendWarpApiCoverage(result);
        return result;
    }

    /// <summary>Desync the TLS of Cloudflare's MASQUE entry points, so the WARP proxy can connect.
    ///
    /// <para>The names are already covered by <see cref="AppendWarpApiCoverage"/> — winws2 matches
    /// subdomains and the tunnel opens under <c>consumer-masque.cloudflareclient.com</c>. What was
    /// missing is STRENGTH, not coverage: that profile carries a deliberately gentle pipeline chosen so
    /// registration stays safe behind a home NAT, and it does not get the tunnel itself through.
    /// Measured on a censored Russian ISP — MASQUE connected only with the bypass scope widened to every
    /// site, i.e. only when the strategy's own full pipeline was the one handling it.</para>
    ///
    /// <para>Aimed by ADDRESS at exactly two /32s, so nothing else on the machine can be caught by it.
    /// The pipeline is the one the owner's own working strategy uses (fake with a negative timestamp,
    /// then a split overlapping a real ClientHello) — proven on the network this exists for, rather than
    /// invented here. The fake is harmless on an uncensored network: a negative timestamp makes the real
    /// server drop it under PAWS while the genuine ClientHello arrives untouched.</para>
    ///
    /// <para>The blob is declared under its own name rather than reusing the strategy's <c>tls_google</c>:
    /// this profile is appended to EVERY preset, including ones that never declared it, and since
    /// zapret2 v1.0.4 an undeclared blob silently stops the whole execution plan — no error, no bypass.
    /// A no-op if either the list or the blob is missing.</para></summary>
    private static void AppendMasqueCoverage(List<string> args)
    {
        string ipset = AppPaths.IpsetFile("masque");
        string blob = Path.Combine(AppPaths.FilesDir, "fake", "tls_clienthello_www_google_com.bin");
        if (!File.Exists(ipset) || !File.Exists(blob)) return;

        // --blob is a GLOBAL declaration: it belongs before anything opens a profile.
        int firstProfileArg = args.FindIndex(IsProfileArg);
        if (firstProfileArg < 0) firstProfileArg = args.Count;
        args.Insert(firstProfileArg, $"--blob={MasqueBlob}:@{blob}");

        args.Add("--new");
        args.Add("--filter-tcp=443-65535");
        args.Add("--filter-l7=tls");
        args.Add($"--ipset={ipset}");
        args.Add("--out-range=-d10");
        args.Add("--payload=tls_client_hello");
        args.Add($"--lua-desync=fake:blob={MasqueBlob}:tcp_ts=-1000:repeats=8");
        args.Add($"--lua-desync=multisplit:pos=1,midsld:seqovl=681:seqovl_pattern={MasqueBlob}:optional");
    }

    /// <summary>Name this profile declares its fake ClientHello under. Deliberately not one of the
    /// strategy's names — see <see cref="AppendMasqueCoverage"/> for why sharing one would be a trap.</summary>
    private const string MasqueBlob = "masque_fake";

    /// <summary>Desync the TLS of Cloudflare's WARP service domains, so registering a device works.
    ///
    /// <para>Registration is an ordinary HTTPS POST to <c>api.cloudflareclient.com</c>, and on a censored
    /// network that name is cut by SNI. Measured rather than assumed: TCP to its address connects in
    /// 51 ms and the TLS handshake then times out — the DPI signature, not a routing problem. In the
    /// default allow-list mode nothing covered that name (it is in neither the YouTube nor the Discord
    /// list, and the catch-all is dropped), so «сгенерировать конфигурацию» failed until the user widened
    /// the scope to every site — a setting they had no reason to connect to WARP at all.</para>
    ///
    /// <para>Unconditional, unlike <see cref="AppendTgProxyCoverage"/>: that one guards a proxy that
    /// already works without it, while this one is the difference between the WARP tab working and not.
    /// It costs one profile, matches by SNI, and nothing but this program talks to those names — so for
    /// a user who never opens the tab it is a filter that never fires. Fooling is the gateway- and
    /// home-NAT-safe pipeline the recommended combo uses, for the same reason it does there: on an
    /// uncensored network the fake segment is rejected by the real server (unkeyed TCP-MD5) and the real
    /// ClientHello arrives untouched. No-op until the list has been seeded.</para></summary>
    private static void AppendWarpApiCoverage(List<string> args)
    {
        string list = AppPaths.WarpApiFile;
        if (!File.Exists(list)) return;
        args.Add("--new");
        args.Add("--filter-tcp=443-65535");
        args.Add("--filter-l7=tls");
        args.Add($"--hostlist={list}");
        args.Add("--out-range=-d10");
        args.Add("--payload=tls_client_hello");
        args.Add("--lua-desync=hostfakesplit:host=www.google.com:tcp_ts=-1000:tcp_md5:repeats=4");
    }

    /// <summary>True for an argument that belongs to a profile rather than the global preamble. Used to
    /// find where the global section ends; <c>--new</c> counts because it opens the first profile.</summary>
    private static bool IsProfileArg(string a) =>
        a == "--new"
        || a.StartsWith("--filter-", StringComparison.Ordinal)
        || a.StartsWith("--payload=", StringComparison.Ordinal)
        || a.StartsWith("--lua-desync=", StringComparison.Ordinal)
        || a.StartsWith("--out-range=", StringComparison.Ordinal)
        || a.StartsWith("--in-range=", StringComparison.Ordinal)
        || a.StartsWith("--hostlist", StringComparison.Ordinal)
        || a.StartsWith("--ipset", StringComparison.Ordinal);

    /// <summary>Append a TLS-desync profile for the built-in Telegram proxy's own upstreams — both the
    /// Cloudflare fronts (<c>kws*.&lt;front&gt;.co.uk</c>) and the direct edge
    /// (<c>kws*.web.telegram.org</c>). Since 2026 the mobile TSPU corrupts the proxy's WebSocket tunnel
    ...
    public void Start(Preset preset, string? hostlistPath)
    {
        lock (_lock)
        {
            ...
            foreach (var a in BuildArguments(preset, hostlistPath, GameFilter, BypassAllSites, DisableQuic,
                                             CoverTgProxy, DebugLog, forLaunch: true))
    ...
    public void Stop()
    {
        ...
        // The process object no longer refers to anything: it exited between the state check and here,
        // and OnProcessExited already tidied up. Nothing went wrong, so nothing is reported — this used
        // to print "Ошибка остановки: No process is associated with this object" on every ordinary
        // restart, which reads as a fault and is not one.
