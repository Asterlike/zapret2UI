...
| **Комбо — Flowseal ALT (fake+fakedsplit)** | `fake` с `tcp_ts` + `fakedsplit` (fooling только на фейках). Пробуйте, если multisplit-вариант не пробивает. |
...
1. **Переберите стратегии вручную** (вкладка «Стратегии» → выбрать → «Применить»), в таком порядке:
   ...
   `Комбо — Flowseal (multisplit seqovl)` → `Комбо — Flowseal ALT (fake+fakedsplit)` →
   `Комбо — окно (wssize)` → `Discord — адаптивный (circular, эксперим.)`.
...
данные — в одной папке:

**`%LOCALAPPDATA%\Zapret2UI\`** (быстро открыть: `Win+R` → вставить путь → Enter)
...
2. Если включали автозапуск, снимите его: Настройки → выключить «Запускать с Windows» (или вручную в
   консоли: `schtasks /delete /tn "Zapret2UI Autostart" /f`).
