...
- **Пресеты** — встроенные стратегии на документированном синтаксисе winws2: «Общий (рекомендуемый)»,
  «YouTube / Google», «Discord (чат + голос)», «Discord — окно (wssize)», «Discord — по IP (ipset)»,
  «Только QUIC» и «Эталон bol-van (всё сразу)». Любой можно продублировать и отредактировать;
  пользовательские хранятся в JSON.
...
1. На вкладке **Управление** выберите пресет (по умолчанию **«Общий (рекомендуемый)»**).
2. Нажмите **Запустить** — индикатор станет зелёным, в **Логах** пойдёт вывод движка.
3. Проверьте доступ к нужному сайту. Если не помогло — вкладка **Проверка** или см. ниже.
...
5. **Перебирайте пресеты вручную.** «Общий», затем «Эталон bol-van»; для сервисов — «YouTube / Google»
   и «Discord (чат + голос)». Если упирается вход в Discord — «Discord — окно (wssize)».
6. **Жёсткая блокировка Discord.** Постройте список IP (вкладка **Настройки** → «Обход по IP») и
   используйте пресет **«Discord — по IP (ipset)»**.
7. **Помните про IP-блокировки сайтов.** Если ресурс режут по IP (а не по имени), обход DPI бессилен —
   нужен VPN/прокси.
