...
- **Простой** (по умолчанию) — один экран и одна кнопка **«Включить обход»**. Применяет
  ...
  (автоподбор) и выбор цели (Discord + YouTube / только Discord / только YouTube). Слева — отдельная
  карточка **Telegram** (доступ через ваш MTProxy в один клик + кнопка «Как это работает»). Ничего
...
- **Пресеты** — встроенные «комбо»-стратегии на документированном синтаксисе winws2. Каждая в одной
  ...
  всегда несёт профиль голоса Discord — переключаться между пресетами не нужно. Список сгруппирован:
  ...
  - **Telegram (через ваш прокси)** — 3 варианта под ваш MTProxy (см. раздел [Telegram](#telegram)).
  - **Личные** — сохранённые автоподбором и ваши копии.

  Деталь пресета показывает понятное описание; сырые аргументы движка спрятаны за кнопкой
  **«Аргументы движка»** (редактор внутри приложения, не правка `.txt`). Любой пресет можно
  продублировать и отредактировать; пользовательские хранятся в JSON. Кнопка **«Как это работает»** под
  списком открывает краткий экскурс по приложению.
- **Хостлисты** — управление списками доменов (txt, один домен на строку), подключение к пресету.
  Встроенные списки (`youtube`/`discord`/`telegram`/`exclude`) **пере-засеваются из приложения при
  каждом запуске**, чтобы обновления доменов доходили до старых установок; ваши собственные списки и
  список **`proxy`** (адрес вашего MTProxy) не трогаются.
...
- **Обход по IP (ipset)** — для жёстких блокировок: резолвит домены Discord через `mdig`,
  ...
  Тем же механизмом адрес вашего **MTProxy** резолвится в IP, чтобы Telegram-пресет десинкал строго
  ваш прокси-сервер и не задевал остальной трафик.
...
Telegram — особый случай, и подход к нему здесь принципиально отличается от Discord/YouTube.

**Что важно понять про приложение Telegram.** Прямое (без прокси) подключение приложения к серверам
Telegram в РФ во многих сетях **не восстанавливается десинком** — по словам автора движка bol-van, это
зависит исключительно от регулятора: работает он «мягко» — можно вытащить, работает «жёстко» — нет.
Поэтому надёжный путь для приложения — **MTProxy** (встроенный в Telegram прокси). А роль этой оболочки —
**помочь вашему клиенту стабильно дозвониться до прокси**.

### Пресеты «Комбо + Telegram через ваш прокси»

Современные MTProxy используют секрет **`ee` (FakeTLS)** — рукопожатие к прокси уходит обычным
TLS-ClientHello к произвольному (поддельному) SNI прокси-сервера. DPI ловит его по отпечатку ClientHello.
Эти пресеты:

- **десинкуют рукопожатие СТРОГО к IP вашего прокси** (по списку `proxy`, см. ниже) — `multidisorder`,
  победивший в тестах. YouTube/Discord/голос и остальной трафик **не затрагиваются** (это полноценное
  рекомендуемое комбо + один профиль под ваш прокси);
- **заодно чинят веб-версию Telegram в браузере** (`web.telegram.org`, `t.me` и т.п. — это обычный TLS
  по SNI, лечится тем же catch-all десинком).

Вариантов **три** (осн. / вариант 2 / вариант 3) — они отличаются только десинком прокси-рукопожатия. На
разных операторах связи заходит разный — **пробуйте**. В тестах основной давал ~60% удачных коннектов к
«трудному» прокси и ~100% к рабочему; оставшиеся неудачи — сторона регулятора, десинком не лечатся.

### Как включить

1. **Простой режим:** карточка **Telegram** слева → **«Включить для Telegram»**. **Расширенный режим:**
   вкладка **Пресеты** → группа **Telegram (через ваш прокси)** → выберите вариант.
2. Появится окно — **введите «Хост» из настроек вашего прокси в Telegram** (Настройки → Данные и память →
   Прокси → ваш прокси → поле «Хост»). Пока хост не задан, кнопка подключения недоступна. Менять хост
   позже — на вкладке **Хостлисты**, список **`proxy`**.
3. Включите обход и **в самом Telegram подключайтесь именно через этот прокси**.

Кнопка **«Как это работает»** (в карточке и под списком пресетов) открывает то же объяснение прямо в
приложении.

> ℹ️ Эти пресеты **не «чинят» сам прокси** и не лечат приложение Telegram напрямую — они помогают до
> прокси дозвониться. Если ни один вариант не пробивает ваш прокси — смените прокси-сервер, либо
> используйте **[tg-ws-proxy](https://github.com/Flowseal/tg-ws-proxy)** (локальный прокси, заворачивает
> MTProto в WebSocket; есть форки под Android) или **Cloudflare WARP**.
...
| `lists\*.txt` | хостлисты (`youtube`/`discord`/`telegram`/`exclude`/`proxy` …) + построенные IP-списки `ipset-*.txt` |
...
- `Services/`:
  ...
  - `EngineService` — запуск/остановка winws2, сборка аргументов (раскрытие токенов
    `{FILES}`/`{WF}`/`{HOSTLIST}` и именованных `{HOSTLIST:name}`/`{EXCLUDE:name}`/`{IPSET:name}`), логи.
  - `PresetService` (комбо + 3 Telegram-прокси-пресета), `HostlistService` (встроенные списки
    пере-засеваются из кода при каждом запуске), `SettingsService`, `AutostartService`, `AppPaths`.
  - `ComboStrategyCatalog` + `AutoSelectService` — пул многопрофильных стратегий и авто-подбор
    лучшей по числу успехов (цель — одна стратегия на YouTube и Discord сразу; Telegram-прокси
    автоподбором не проверяется — TLS-проба не отражает работу нативного клиента).
  - `DiagnosticsService` + `NetProbe` — матрица доступности точек (ping / HTTP / TLS 1.2 / TLS 1.3).
  - `IpsetService` — построение IP‑списка Discord (`mdig` → `ip2net`) и резолв хоста MTProxy в
    `ipset-proxy.txt` (через DNS, без внешних тулов).
...
- `MainWindow.xaml` — окно: кастомный заголовок, два режима (Простой/Расширенный), боковая навигация,
  6 разделов (Управление, Пресеты, Хостлисты, Проверка, Логи, Настройки) + модальные окна (ввод хоста
